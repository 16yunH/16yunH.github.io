...
\begin{cventry}{World-Action Models for Robotic Manipulation}{Current research}{MagicLab, Fudan University \textbar{} Advisor: Wenchao Ding}
\begin{cvitems}
\item Developing an execution-feedback framework around LingBot-VA on LIBERO-Long; studying how task progress and predicted-future discrepancies can condition the same policy to revise actions.
\item Using state replay and controlled interventions to distinguish changes in model outputs from task recovery, and examine the roles of arm/gripper configurations, reachability, and streaming observation history at subtask handoffs.
\item Investigating how favorable simulated starting states can be reached through safe, executable actions for subsequent WAM-controlled manipulation.
\end{cvitems}
\end{cventry}
